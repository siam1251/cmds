...
|cassandra,hbase, big table| wide column store, key-value, use row key| typically used as eventually consistent| [hbase](https://hbase.apache.org/book.html#datamodel) has column family then colum name, usually one hfile per column family, row_key, column-family[c1, c2] are sorted based on row_key. searching based on row_key is fast but with other keys it's slow, you may create another  table with another row_key for faster search|
